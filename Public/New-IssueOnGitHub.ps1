function New-IssueOnGitHub {
<#
.SYNOPSIS
Creates a new GitHub Issue with title, multiline body, optional assignees, labels, and milestone.

.DESCRIPTION
This function allows you to interactively create a new GitHub Issue with:
- Multiline text input via editor
...
.EXAMPLE
New-IssueOnGitHub -Owner 'theMompfdie' -Repo 'IceSelfCheckIn' -GitHubToken 'your_token_here'
...
#>
    ...
    # Title
    $Title = Read-Host 'Enter the title of the new issue'

    # Body via editor
    $TempFile = [System.IO.Path]::GetTempFileName()
    $BodyContent = Read-MultilineInputWithEditor -TempFilePath $TempFile

    # Assignees
    $Assignees = @()
    $SetAssignees = Read-Host 'Do you want to assign users? (yes/no)'
    if ($SetAssignees -eq 'yes') {
        $AssigneesInput = Read-Host 'Enter assignee usernames (comma separated)'
        $Assignees = $AssigneesInput -split ',' | ForEach-Object { $_.Trim() }
    }

    # Labels
    $Labels = @()
    $SetLabels = Read-Host 'Do you want to add labels? (yes/no)'
    if ($SetLabels -eq 'yes') {
        ...
        Write-Host "Available labels:" -ForegroundColor Cyan
        $AvailableLabels | ForEach-Object { Write-Host "- $($_.name)" }

        $LabelsInput = Read-Host 'Enter labels (comma separated, exactly as shown)'
        $Labels = $LabelsInput -split ',' | ForEach-Object { $_.Trim() }
    }

    # Milestone
    $MilestoneId = $null
    $MilestonesUri = "https://api.github.com/repos/$Owner/$Repo/milestones?state=open"
    $Milestones = (Invoke-RestMethod -Uri $MilestonesUri -Headers $Headers -Method Get)

    if ($Milestones) {
        Write-Host "Available milestones:" -ForegroundColor Cyan
        $Milestones | ForEach-Object { Write-Host ("[{0}] {1}" -f $_.number, $_.title) }

        $MilestoneChoice = Read-Host 'Enter milestone number to assign (or leave blank)'
        if ($MilestoneChoice) {
            $MilestoneId = [int]$MilestoneChoice
        }
    }
    ...
    if ($MilestoneId) {
        $IssueBody['milestone'] = $MilestoneId
    }

    $IssueUri = "https://api.github.com/repos/$Owner/$Repo/issues"
    $BodyJson = $IssueBody | ConvertTo-Json -Depth 5

    $Response = Invoke-RestMethod -Uri $IssueUri -Headers $Headers -Method Post -Body $BodyJson

    if ($Response) {
        Write-Host "✅ Issue '$Title' created successfully!" -ForegroundColor Green
        Write-Host "🔗 Link: $($Response.html_url)" -ForegroundColor Cyan
    }
    else {
        Write-Host "❌ Failed to create the issue." -ForegroundColor Red
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUazcLGQUbVhJzjLT8vDkkEX3V
# zJOggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWB