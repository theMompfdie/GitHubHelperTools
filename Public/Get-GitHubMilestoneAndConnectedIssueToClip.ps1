function Get-GitHubMilestoneAndConnectedIssueToClip {
<#
.SYNOPSIS
Fetches a GitHub milestone and its associated issues and copies a formatted summary to the clipboard.

.DESCRIPTION
This function retrieves a specific milestone and all its assigned issues from a GitHub repository.
The result is copied to the clipboard, formatted nicely for documentation or project tracking.
...
The number (ID) of the milestone to fetch.

.PARAMETER GitHubToken
A GitHub personal access token (PAT) for authentication (mandatory for private repositories).

.EXAMPLE
Get-GitHubMilestoneAndConnectedIssueToClip -Owner 'theMompfdie' -Repo 'IceSelfCheckIn' -MilestoneNumber 2 -GitHubToken 'your_token_here'
...
#>
    ...
    # URLs
    $MilestoneUrl = "https://api.github.com/repos/$Owner/$Repo/milestones/$MilestoneNumber"
    $IssuesUrl = "https://api.github.com/repos/$Owner/$Repo/issues?milestone=$MilestoneNumber&state=all&per_page=100"

    try {
        # Fetch milestone
        $Milestone = Invoke-RestMethod -Uri $MilestoneUrl -Headers $Headers -Method Get
...
"@
        ...
        foreach ($Issue in $Issues) {
            $Text += "`n- #$($Issue.number) [$($Issue.title)]($($Issue.html_url))"
        }

        # Copy to clipboard
        $Text | Set-Clipboard

        Write-Host "✅ Milestone and associated issues have been copied to clipboard." -ForegroundColor Green
    }
    catch {
        Write-Host "❌ Error fetching milestone or issues. Please check your credentials and milestone number." -ForegroundColor Red
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUxT5+QKWj8XT7hX2/WfIzhV9R
# toOggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBSqoPNl
# u+k4QNsVth3YFEOIbhkb5DANBgkqhkiG9w0BAQEFAASCAQBdoUL//1eXOgEixB8p
# cGdTyD/jmLdyxRp898TABaB3U+RDYbgkVfYx3JB031VPfRl6V4ozX+DpjNmgpu7A
# nebXvGQtmDC4GB0bHCIgQFUBRZsm99qhisAISz/QbasClJ5gXM82cWmIdYjxWcs2
# HjgVWtmQCjyj6Hfd+H9IgeEs4rHdsY31N6duesM+/kc0GhHkSDKo7KAoPX0bx1Ja
# mAQ8Ruf8Xk6D7FFc1Z3nRZexpk4rDOFBJXjSoX1tOD7gS3X9tjttniECKN2lGoDc
# dXRfIsxjXXPc4OPhDLyS4t6Yj1vAlGA/lsdv3TNeyFzMzA+Fdspzjd6ijxwg4ib1
# R2oIoYIDIDCCAxwGCSqGSIb3DQEJBjGCAw0wggMJAgEBMHcwYzELMAkGA1UEBhMC
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1MDQyODIwMjE0MFowLwYJKoZIhvcNAQkE
# MSIEIP1VRV/q5K/nsA3/TGTb2/DkGqcB/XtFL5fqibqOxjgPMA0GCSqGSIb3DQEB
# AQUABIICADUcIti+0QKkB2SBT5SrN0g9cnsXdEdN8AcslJP7O+417Q6UFY7KVVBm
# l411DYwyHbu/XnSxRk9BpPGa6saXd4tXpB9Jf/vdsHTXEOdukbvi8e7O7cdzh+YJ
# VdWMP2wul+toAYvv8jnFqS/hh9VXpkvpU2Ewabm/KJZCgV6nzYTjRIATQVtgbsrC
# RQeER1tHunOG5mZLVCz1VgaBVxsKKcTxvVTj31wRrhmOFzpQA7tVHDHQ17xZj3pp
# /b9RjP6ofFTvLACxWlz51tMYg2k0ouqrtP7/96AkrcUwKDvTrK/nxtpDdB2l0Xyr
# O+tEiuecABbN48sHjv//3CQi3iArqsjhJlCsW/aTT/alYYaPd9MREycvWdujguj2
# ppJKSWLvcCCkR1yA5DK9O8cUxxzaUFPF06QOySpxQEAGldSM/3SMJjVRX6NuIjF4
# 4hFdaEQqILo45lAfBxSZn87S1BzWb7vj+W7TYJkP0MoOOfVFt1SL0fbBirqOoueK
# Cf8CimMpUF84Gr9behVeT55bFEtcRkKptIG0ZNqwJv62kbrdU9gkj3YFSN8OlfQE
# AiY+4L15glCNh4SJvivCTpdCXwZjuXmkiRB/b4Uy8q6470mB4KqFALFhPj6xLGle
# cZUCk6X3u75DlDgo80dnyijJQ4XDl5+Ez+VRGWa69JSuwY8FUTLb
