function New-LabelOnGitHub {
<#
.SYNOPSIS
Creates a new label in a GitHub repository, ensuring no duplicates and choosing a distinct color.

.DESCRIPTION
This function allows you to create a new label in a specified GitHub repository.
It checks if the label already exists, ensures the color is unique, and allows adding a description.
If all predefined colors are used, a random hex color will be generated.
...
.EXAMPLE
New-LabelOnGitHub -Owner 'theMompfdie' -Repo 'IceSelfCheckIn' -GitHubToken 'your_token_here'
...
#>
    ...
    $ExistingLabels = (Invoke-RestMethod -Uri $LabelsUri -Headers $Headers -Method Get)

    # Prompt for label name
    $NewLabelName = Read-Host 'Enter the name of the new label'

    # Check if label already exists
    if ($ExistingLabels.name -contains $NewLabelName) {
        Write-Host "❌ Label '$NewLabelName' already exists!" -ForegroundColor Red
        return
    }

    # Prompt for optional description
    $NewLabelDescription = Read-Host 'Enter a description for the label (optional)'

    # Predefined color list
    $PossibleColors = @(
        'FF5733','33FF57','3357FF','FF33A8','A833FF','33FFF5','FF8C33','FFD633','A6FF33','33FFA6',
        'FF3333','33FF8C','8C33FF','FF33FF','33D1FF','FF6F33','D6FF33','33FFD6','FF33D1'
    )

    $UsedColors = $ExistingLabels.color
    $AvailableColors = $PossibleColors | Where-Object { $_ -notin $UsedColors }

    if (-not $AvailableColors) {
        Write-Host "⚠️ All predefined colors are used. Generating a fully random color..." -ForegroundColor Yellow
        $RandomColor = -join ((0..5 | ForEach-Object { ('0'..'9') + ('A'..'F') | Get-Random }) )
    }
    else {
        $RandomColor = $AvailableColors | Get-Random
    }
    ...
    $Response = Invoke-RestMethod -Uri $CreateUri -Headers $Headers -Method Post -Body $NewLabelBody

    if ($Response) {
        Write-Host "✅ Label '$NewLabelName' successfully created with color #$RandomColor." -ForegroundColor Green
    }
    else {
        Write-Host "❌ Failed to create the label." -ForegroundColor Red
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5ghjdOm9A3mdCFUhHLKprfEF
# IiuggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQYg8u9
# l7oZcYV2QubbyVQyM0m1IDANBgkqhkiG9w0BAQEFAASCAQCe+fSlvuS+f7qMnFcN
# diNrDyNFSsO/fMtkZ30QItMpvVF5sUADiXpGO/HN80ZS7HP1LHp9FOo1wNSNdpMv
# pjzwhX4oJym68ummqN1CgE0u9i+t9heXSGrnl5xXRUZL6S1gFHu0xh/qY5prLNz9
# k7AkKGZxV1okiK/b6avXL/gPOjUg5vW8ndM1nBaHhRxbBrIApzd+al0+CA/fcC3b
# BvzR0KwjNPucIldA+PwcduVXUqxdGaYDdt/73JAhW6hJVP9GP3As1RyQIR33CM6u
# y3ksBzLIDh3RogCN1oG7GJ8ts7gk09x+H/PrJmVORobyOZB8EY7Lz42A8TcgALzW
# VzKCoYIDIDCCAxwGCSqGSIb3DQEJBjGCAw0wggMJAgEBMHcwYzELMAkGA1UEBhMC
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1MDQyODIwMTczM1owLwYJKoZIhvcNAQkE
# MSIEINNXhels0jaqU2u5ooqDKzctnzLJ0+8WkSpoCWo/0sAfMA0GCSqGSIb3DQEB
# AQUABIICAAcdpMbMThXzmIzykTj9V3yPz4F5E8PBthhWecjPMgpvT9h5WtwyJh3r
# T+GRHqicirrelLw+UzUxrha8kCbo4Sw2Q+je/FPVFiB1exdWSNH9opgpRPk2mmtS
# 3gWyP2EJfLXPIkXYujuo6OXaOb+PaOM0kqC9JvsNens4swCU/0kvv9UQ6m3eMthK
# Y+r/Dal51dZLTVPo9BUGlk5tay8sRqytcJYEBujsnzFC1mT4SKhm8BJce0EI3MXS
# DZsQXdUCI/RzeQ6sgOIJvjnDc9MP+HZpM46VD0aM1z3CZua53/rEqkKgTFZJADR2
# 7DUa9NUj2yMqr88zhP0eVNl5yQcpNYoRxSnGM59oyLasT+aPdYDBgSmX/s+fWJVc
# Z+S6hT+qgM9wo3WpPeVSVJNwyrSnu5asBds3wFCUYV971C+bPbYuefan5xOd5vea
# lwsJOkRPc4WOQdvL3sZMtXdCAivMXhnszUR5JG58vrrQG6VQibphyfoTie+RxP8l
# r9TSqaQtYtUdDdh1aw5vMyPGusLxU6gaevx22PSpq3DVjwHxPZCcZOtwtfiZehT8
# 3KlTmKyapw7zDma8pivYJtqIZ5EO+Db9OKRnYIV2EwFpbGJfuGQnLIzsS4LS/N0h
# qrN7SjiyVkRbPgwqkjZsAyLrksUa3RbsrhmiSy6SmmetF+EHmU52
