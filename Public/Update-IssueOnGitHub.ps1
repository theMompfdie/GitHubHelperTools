function Update-IssueOnGitHub {
<#
.SYNOPSIS
Updates an existing GitHub Issue by adding comments, updating labels, assigning milestones, and optionally closing it.

.DESCRIPTION
This function allows you to interactively update an existing GitHub Issue:
- Add a new comment (with multiline editor support)
- Update labels (multiple selection possible)
- Assign the issue to yourself if not already assigned
- Set or update the milestone
- Close the issue if desired
...
A GitHub personal access token (PAT) with repo scope for authentication.

.EXAMPLE
Update-IssueOnGitHub -Owner 'theMompfdie' -Repo 'IceSelfCheckIn' -GitHubToken 'your_token_here'
...
#>
    ...
    $IssueNumber = Read-Host 'Please enter the issue number'

    # Add a comment?
    $addComment = Read-Host 'Do you want to add a comment? (yes/no)'
    if ($addComment -eq 'yes') {
        $TempFile = [System.IO.Path]::GetTempFileName()
        $CommentBody = Read-MultilineInputWithEditor -TempFilePath $TempFile

        if ($CommentBody.Trim()) {
            $CommentUri = "https://api.github.com/repos/$Owner/$Repo/issues/$IssueNumber/comments"
            $CommentBodyJson = @{ body = $CommentBody } | ConvertTo-Json -Depth 3
            Invoke-RestMethod -Uri $CommentUri -Headers $Headers -Method Post -Body $CommentBodyJson
            Write-Host "✅ Comment added to Issue #$IssueNumber." -ForegroundColor Green
        }
        else {
            Write-Host "➡️ No comment added."
        }
    }
    else {
        Write-Host "➡️ No comment added."
    ...
    # Update labels?
    $updateLabels = Read-Host 'Do you want to update labels? (yes/no)'
    if ($updateLabels -eq 'yes') {
        $LabelsUri = "https://api.github.com/repos/$Owner/$Repo/labels"
        $AvailableLabels = Invoke-RestMethod -Uri $LabelsUri -Headers $Headers -Method Get

        $AvailableLabels | ForEach-Object { Write-Host $_.name }

        $SelectedLabels = Read-Host 'Enter labels separated by commas (exactly as shown)'
        $LabelArray = $SelectedLabels -split ',' | ForEach-Object { $_.Trim() }

        $IssueUri = "https://api.github.com/repos/$Owner/$Repo/issues/$IssueNumber"
        $UpdateLabelsBodyJson = @{ labels = $LabelArray } | ConvertTo-Json -Depth 3
        Invoke-RestMethod -Uri $IssueUri -Headers $Headers -Method Patch -Body $UpdateLabelsBodyJson
        Write-Host "✅ Labels updated." -ForegroundColor Green
    }
    else {
        Write-Host "➡️ No label update."
    }

    # Assign to yourself if not assigned yet
    $AssignUri = "https://api.github.com/repos/$Owner/$Repo/issues/$IssueNumber"
    $IssueData = Invoke-RestMethod -Uri $AssignUri -Headers $Headers -Method Get

    if (-not $IssueData.assignee) {
        $AssignBody = @{ assignees = @($Owner) } | ConvertTo-Json -Depth 3
        Invoke-RestMethod -Uri $AssignUri -Headers $Headers -Method Patch -Body $AssignBody
        Write-Host "✅ Issue assigned to $Owner." -ForegroundColor Green
    }
    else {
        Write-Host "➡️ Issue already has an assignee."
    }

    # Update milestone?
    $updateMilestone = Read-Host 'Do you want to update milestone? (yes/no)'
    if ($updateMilestone -eq 'yes') {
        $MilestonesUri = "https://api.github.com/repos/$Owner/$Repo/milestones"
        $AvailableMilestones = Invoke-RestMethod -Uri $MilestonesUri -Headers $Headers -Method Get

        foreach ($milestone in $AvailableMilestones) {
            Write-Host "$($milestone.number): $($milestone.title)"
        }

        $SelectedMilestoneNumber = Read-Host 'Enter milestone number to assign (or leave empty to skip)'
        if ($SelectedMilestoneNumber) {
            $UpdateMilestoneBodyJson = @{ milestone = [int]$SelectedMilestoneNumber } | ConvertTo-Json -Depth 3
            Invoke-RestMethod -Uri $IssueUri -Headers $Headers -Method Patch -Body $UpdateMilestoneBodyJson
            Write-Host "✅ Milestone updated." -ForegroundColor Green
        }
        else {
            Write-Host "➡️ No milestone updated."
        }
    }
    else {
        Write-Host "➡️ No milestone update."
    }

    # Close issue?
    $closeIssue = Read-Host 'Do you want to close the issue? (yes/no)'
    if ($closeIssue -eq 'yes') {
        $CloseIssueBodyJson = @{ state = 'closed' } | ConvertTo-Json -Depth 3
        Invoke-RestMethod -Uri $IssueUri -Headers $Headers -Method Patch -Body $CloseIssueBodyJson
        Write-Host "✅ Issue #$IssueNumber closed." -ForegroundColor Green
    }
    else {
        Write-Host "➡️ Issue left open."
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUwIDfcwaKNWDT1NlyZ1ltyVan
# 6AaggiT/MIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBSMrFM8
# qneez8Uzb1GuEj4WN2hhxTANBgkqhkiG9w0BAQEFAASCAQBnpzMDdAQ0Q+WK3Ic0
# y9YimYewFPVC5ZBKvoNKx1ASpnqk4MEyI5JBot6644OycMzl/JOKGKUKXAm8AO02
# yQGwONZTCUebc9Bdr2MYNXGEnAL1E/qBWaHeoBGTEhYX51aMMN5GP1AiEpAvZO6w
# r1QFRyA6HwX6rUna/NWH0K1fI4bOIk0/yPTfffvzrJeqDSe8UkHb3smcokFNe35J
# 18WMXRhQcaSgtNcHf6W/gFm7Ww5i3wXblZcQzRs5UAMYkFVbes6TvYq0uiw+cU5o
# Fhru7jn11qXVAWESUiMCdpHVUihV10oOMtD3Dlc75LB7QRnxN3Yi5ozkJVKdId+c
# bKWQoYIDIDCCAxwGCSqGSIb3DQEJBjGCAw0wggMJAgEBMHcwYzELMAkGA1UEBhMC
...
# 9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1MDQyODIwMTM0OFowLwYJKoZIhvcNAQkE
# MSIEIC62xkVgxTQSYEoO+s7AdA7Mvt7V9Bk8X6ZtipgzOIVnMA0GCSqGSIb3DQEB
# AQUABIICAB+CGFIY6XACpEgO7+YcyM6kArSFLFzqILlyVxJarTomTmnhd+KXcOmD
# fKz1gkSZZ9YMqjWvb5ySLaV5L+BkoyjC4btHpxhVmgRv4liJX/MPCToZQ8QVU5Hu
# jOQmMU0JFKR+7xHBIiNaFnXrfEu1cOdvPPZoAWoMhMazLYgn1MLPaIzcVkjZdFbv
# Xot4HHTJD4iqxYGyp3cao4lJoa5dxtAU+UKQRAnT5IAdepS4X27zeQQboGZQzihY
# 4aRBe+rS2w2qO9eq5yYElKPFWEZy+QuLFRpJbOaXvrRZLSJTAas2///LAG8mydHT
# k01h//jxYi5xc+aKMQl33DpYAP3x+UdNb8vgPoUXjA9UvaL+pvVb6CGClnBPjvUv
# XtU+EtZMRGW6oy9a3dvKPZoFG5fNGidIQqeWw7w3AOWWxWd8hxhqCgcA229LHreq
# nBEbeHmUsaI3rGuSmtxRtL3RoUembWmL1Rh4pnmJ88vZPNtYjR+2yagE2/XZgWDe
# 3041vkg5+XImFYHFR6B2oauMPYGbF542UuXz0TAdcKl9zpr5rcQW1EAOUs7JG8Cq
# EaIsHaN1llBUXRap4jKNT39P5cq24YCC9uDZ6XoczSup6/pn5a6i05JTSnH6f2PQ
# vt5eNusSJi9bN+q1JtXGEYkPLqw3Rf3NrRgpQmN14xn+35OF+jlg
